...
class StripeService
{
    ...
    public function handleApproval()
    {
        if (session()->has('paymentIntentId')) {
            ...
            if ($confirmation->status === 'succeeded') {
                ...
                $amount = strtoupper($confirmation->amount / $this->resolveFactor($currency));

                return redirect()->route('home')->withSuccess(['payment' => "Thanks, {$name}. We received your {$amount} {$currency} payment."]);;
            }
        }
        return redirect()->route('home')->withErrors('We were unable to confirm you payment. Try again, please');
    }
    ...
    public function createIntent($value, $currency, $paymentMethod)
    {
        return $this->makeRequest(
            ...
            [],
            [
                'amount' => round($value * $this->resolveFactor($currency), 0),
